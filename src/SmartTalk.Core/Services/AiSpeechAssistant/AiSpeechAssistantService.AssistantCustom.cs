...
public partial class AiSpeechAssistantService
{
    ...
    private async Task<Agent> AddAgentAsync(int? relateId, int posServiceId, AgentType type, AgentSourceSystem sourceSystem, bool isDisplay, CancellationToken cancellationToken)
    {
        var agent = new Agent
        {
            Type = type,
    ...
    private async Task<(Agent Agent, NumberPool Number)> InitialRestaurantInternalAsync(AddAiSpeechAssistantCommand command, CancellationToken cancellationToken)
    {
        ...
        var agent = await AddAgentAsync(restaurant.Id, command.PosServiceId, command.AgentType, command.SourceSystem, command.IsDisplay, cancellationToken);
    ...
    {
        var agent = await AddAgentAsync(null, command.PosServiceId, command.AgentType, command.SourceSystem, command.IsDisplay, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<(Agent Agent, NumberPool Number)> InitialPosCompanyStoreInternalAsync(AddAiSpeechAssistantCommand command, CancellationToken cancellationToken)
    {
        ...
        var agent = await AddAgentAsync(null, command.PosServiceId, AgentType.Assistant, command.SourceSystem, command.IsDisplay, cancellationToken).ConfigureAwait(false);
