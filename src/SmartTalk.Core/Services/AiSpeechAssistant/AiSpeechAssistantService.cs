...
public partial class AiSpeechAssistantService : IAiSpeechAssistantService
{
    ...
    private async Task ProcessRepeatOrderAsync(WebSocket twilioWebSocket, JsonElement jsonDocument, CancellationToken cancellationToken)
    {
        ...
        var voice = Enum.Parse<AiSpeechAssistantVoice>(_aiSpeechAssistantStreamContext.Assistant.ModelVoice);
        
        var holdOn = RepeatOrderHoldOnConstants.ScriptLibrary.Where(x => x.Voice == voice).ToList().GetRandomElement();
        
        var holdOnAudio = new
        {
            @event = "media",
            streamSid = _aiSpeechAssistantStreamContext.StreamSid,
            media = new { payload = holdOn.Audio }
        };
