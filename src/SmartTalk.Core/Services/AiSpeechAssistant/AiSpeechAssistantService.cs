...
public class AiSpeechAssistantService : IAiSpeechAssistantService
{
    ...
    private  WebSocket _twilioWebSocket;
    ...
    private void CheckWebSocketState(object sender, ElapsedEventArgs e)
    {
        if (_openAiWebSocket != null)
            Log.Information("Monitoring the openai websocket state: {@State}", _openAiWebSocket.State);
    }
    ...
    public async Task<AiSpeechAssistantConnectCloseEvent> ConnectAiSpeechAssistantAsync(ConnectAiSpeechAssistantCommand command, CancellationToken cancellationToken)
    {
        ...
        _twilioWebSocket = command.TwilioWebSocket;
