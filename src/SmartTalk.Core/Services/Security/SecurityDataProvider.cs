...
public partial interface ISecurityDataProvider : IScopedDependency
{
    ...
    Task<List<RoleUser>> GetRoleUserByRoleNameAsync(string roleName, CancellationToken cancellationToken);
    
    Task<List<Permission>> GetPermissionsByPermissionLevelAsync(PermissionLevel permissionLevel , CancellationToken cancellationToken);
}
