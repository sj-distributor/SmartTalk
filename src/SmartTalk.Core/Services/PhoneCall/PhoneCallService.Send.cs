...
{
    Task DailyDataBroadcastAsync(SchedulingPhoneCallDailyDataBroadcastCommand command, CancellationToken cancellationToken);   
}

public partial class PhoneCallService
{
    public async Task DailyDataBroadcastAsync(SchedulingPhoneCallDailyDataBroadcastCommand command, CancellationToken cancellationToken)
    {
    ...
    private async Task<string> GenerateDailyDataReportAsync(DateTimeOffset today, DateTimeOffset yesterday, CancellationToken cancellationToken)
    {
        ...
        var restaurantCount = await _phoneCallDataProvider
    ...
    private async Task<string> GenerateCustomerServiceAssessmentPeriodReportAsync(DateTimeOffset today, DateTimeOffset yesterday, CancellationToken cancellationToken)
    {
        ...
        var userHasProofreadTheNumber = await _phoneCallDataProvider.GetPhoneCallRecordsWithUserCountAsync(
            previous20Th, nowDate, cancellationToken).ConfigureAwait(false);
        
        var userTodayHasProofreadTheNumber = await _phoneCallDataProvider.GetPhoneCallRecordsWithUserCountAsync(
