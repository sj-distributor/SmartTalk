...
{
    Task<GetPhoneCallRecordsResponse> GetPhoneOrderRecordsAsync(GetPhoneCallRecordsRequest request, CancellationToken cancellationToken);

    Task ReceivePhoneOrderRecordAsync(ReceivePhoneCallRecordCommand command, CancellationToken cancellationToken);

    Task ExtractPhoneOrderRecordAiMenuAsync(List<SpeechMaticsSpeakInfoDto> phoneOrderInfo, PhoneCallRecord record, byte[] audioContent, CancellationToken cancellationToken);
...
{
    public async Task<GetPhoneCallRecordsResponse> GetPhoneOrderRecordsAsync(GetPhoneCallRecordsRequest request, CancellationToken cancellationToken)
    {
        var records = await _phoneCallDataProvider.GetPhoneCallRecordsAsync(request.Restaurant, cancellationToken).ConfigureAwait(false);

        return new GetPhoneCallRecordsResponse
        {
            Data = _mapper.Map<List<PhoneCallRecordDto>>(records)
        };
    }

    public async Task ReceivePhoneOrderRecordAsync(ReceivePhoneCallRecordCommand command, CancellationToken cancellationToken)
    {
        ...
        var record = new PhoneCallRecord { SessionId = Guid.NewGuid().ToString(), Restaurant = recordInfo.Restaurant, TranscriptionText = transcription, Language = SelectLanguageEnum(detection.Language), CreatedDate = recordInfo.OrderDate.AddHours(-8), Status = PhoneCallRecordStatus.Recieved };

        if (await CheckPhoneOrderRecordDurationAsync(command.RecordContent, cancellationToken).ConfigureAwait(false))
        {
            await AddPhoneOrderRecordAsync(record, PhoneCallRecordStatus.NoContent, cancellationToken).ConfigureAwait(false);
        ...
        await AddPhoneOrderRecordAsync(record, PhoneCallRecordStatus.Diarization, cancellationToken).ConfigureAwait(false);
    }

    private async Task<bool> CheckOrderExistAsync(DateTimeOffset createdDate, CancellationToken cancellationToken)
    {
        return (await _phoneCallDataProvider.GetPhoneCallRecordAsync(createdDate: createdDate, cancellationToken: cancellationToken).ConfigureAwait(false)).Any();
    }
    ...
    public async Task ExtractPhoneOrderRecordAiMenuAsync(
        List<SpeechMaticsSpeakInfoDto> phoneOrderInfo, PhoneCallRecord record, byte[] audioContent, CancellationToken cancellationToken)
    {
        ...
        await _phoneCallUtilService.ExtractPhoneOrderShoppingCartAsync(goalText, record, cancellationToken).ConfigureAwait(false);
        
        record.Tips = tip;
        record.Status = PhoneCallRecordStatus.Sent;
    ...
    public async Task<AddOrUpdateManualOrderResponse> AddOrUpdateManualOrderAsync(AddOrUpdateManualOrderCommand command, CancellationToken cancellationToken)
    {
        ...
        var items = await _phoneCallDataProvider.GetPhoneOrderOrderItemsAsync(command.RecordId, PhoneCallOrderType.ManualOrder, cancellationToken).ConfigureAwait(false);

        if (items is { Count: > 0 })
            await _phoneCallDataProvider.DeletePhoneOrderItemsAsync(items, cancellationToken: cancellationToken).ConfigureAwait(false);
        
        var oderItems = manualOrder.Data.Order.OrderItems.Select(x =>
        {
            return new PhoneCallOrderItem
            {
                Price = x.ItemAmount,
                Quantity = x.Quantity,
        ...
        var record = (await _phoneCallDataProvider.GetPhoneCallRecordAsync(command.RecordId, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
        ...
        {
            Data = _mapper.Map<List<PhoneCallOrderItemDto>>(oderItems)
        };
    }
    ...
    private async Task<List<SpeechMaticsSpeakInfoDto>> HandlerConversationFirstSentenceAsync(List<SpeechMaticsSpeakInfoDto> phoneOrderInfos, PhoneCallRecord record, byte[] audioContent, CancellationToken cancellationToken)
    {
        ...
        foreach (var phoneOrderInfo in phoneOrderInfos)
        {
            ...
            phoneOrderInfo.Role = phoneOrderInfo.Role == PhoneCallRole.Restaurant ? PhoneCallRole.Client : PhoneCallRole.Restaurant;
        }
        ...
        List<SpeechMaticsSpeakInfoDto> phoneOrderInfo, PhoneCallRecord record, byte[] audioContent, CancellationToken cancellationToken)
    {
        ...
        foreach (var speakDetail in phoneOrderInfo)
        {
            ...
            try
            {
                ...
                goalTexts.Add((speakDetail.Role == PhoneCallRole.Restaurant
                    ? PhoneCallRole.Restaurant.ToString()
                    : PhoneCallRole.Client.ToString()) + ": " + originText);

                if (speakDetail.Role == PhoneCallRole.Restaurant)
                    conversations.Add(new PhoneCallConversation { RecordId = record.Id, Question = originText, Order = conversationIndex });
                else
                {
        ...
        if (await CheckRestaurantRecordingRoleAsync(goalTextsString, cancellationToken).ConfigureAwait(false))
        {
            ...
            {
                conversations.Insert(0, new PhoneCallConversation
                {
                    Order = 0,
                    Answer = "",
                    Question = "",
        ...
        await _phoneCallDataProvider.AddPhoneOrderConversationsAsync(conversations, true, cancellationToken).ConfigureAwait(false);

        return (goalTextsString, conversations.First().Question);
    }

    private static string ProcessConversation(List<PhoneCallConversation> conversations)
    {
        var goalTextsString = "";
    ...
    private static void ShiftConversations(List<PhoneCallConversation> conversations)
    {
    ...
    private async Task AddPhoneOrderRecordAsync(PhoneCallRecord record, PhoneCallRecordStatus status, CancellationToken cancellationToken)
    {
        record.Status = status;
        
        await _phoneCallDataProvider.AddPhoneCallRecordsAsync(new List<PhoneCallRecord> { record }, cancellationToken: cancellationToken).ConfigureAwait(false);
    }
    ...
    private PhoneCallRecordInformationDto ExtractPhoneOrderRecordInfoFromRecordName(string recordName, string restaurant)
    {
        ...
        return new PhoneCallRecordInformationDto
        {
            OrderDate = DateTimeOffset.FromUnixTimeSeconds(long.Parse(time)),
            Restaurant = (PhoneCallRestaurant)Enum.Parse(typeof(PhoneCallRestaurant), restaurant, true)
        };
    }
    ...
    {
        var record = (await _phoneCallDataProvider.GetPhoneCallRecordAsync(recordId, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
    ...
    public async Task<string> SplitAudioAsync(byte[] file, PhoneCallRecord record, double speakStartTimeVideo, double speakEndTimeVideo, TranscriptionFileType fileType = TranscriptionFileType.Wav, CancellationToken cancellationToken = default)
    {
    ...
    private string SelectPrompt(PhoneCallRestaurant restaurant)
    {
        return restaurant switch
        {
            PhoneCallRestaurant.MoonHouse => "Moon, Hello Moon house, Moon house",
            PhoneCallRestaurant.XiangTanRenJia => "你好,湘里人家",
            PhoneCallRestaurant.JiangNanChun => "你好,江南春",
            _ => ""
        };
    }
