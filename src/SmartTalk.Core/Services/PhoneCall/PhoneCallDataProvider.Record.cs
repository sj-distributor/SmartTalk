...
public partial class PhoneCallDataProvider
{
    ...
    public async Task<List<PhoneCallRecord>> GetPhoneCallRecordsAsync(PhoneCallRestaurant restaurant, CancellationToken cancellationToken)
    {
        var query = _repository.Query<PhoneCallRecord>()
            .Where(record => record.Restaurant == restaurant && record.Status == PhoneCallRecordStatus.Sent)
        ...
        int? recordId = null, DateTimeOffset? createdDate = null, CancellationToken cancellationToken = default)
    {
        ...
        if (createdDate.HasValue)
            query = query.Where(x => x.CreatedDate == createdDate && x.Status == PhoneCallRecordStatus.Sent);
        ...
        DateTimeOffset dayShiftTime, DateTimeOffset nightShiftTime, DateTimeOffset endTime, CancellationToken cancellationToken)
    {
        return await _repository.Query<PhoneCallRecord>()
            .Where(x => x.Status == PhoneCallRecordStatus.Sent)
