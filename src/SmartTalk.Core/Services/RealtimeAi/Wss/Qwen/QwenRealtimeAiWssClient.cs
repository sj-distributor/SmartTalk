...
public class QwenRealtimeAiWssClient : IRealtimeAiWssClient
{
    ...
    public async Task ConnectAsync(Uri endpointUri, Dictionary<string, string> customHeaders, CancellationToken cancellationToken)
    {
        ...
        try
        {
            Log.Information("Qwen Realtime Wss Client: Connecting to {EndpointUri}...", EndpointUri);
            await _webSocket.ConnectAsync(EndpointUri, cancellationToken).ConfigureAwait(false);
            Log.Information("Qwen Realtime Wss Client: Successfully connected to {EndpointUri}. State: {State}", EndpointUri, _webSocket.State);
        ...
        {
            Log.Error(ex, "Qwen Realtime Wss Client: Failed to connect to {EndpointUri}.", EndpointUri);
            await (ErrorOccurredAsync?.Invoke(ex) ?? Task.CompletedTask);
            await (StateChangedAsync?.Invoke(_webSocket?.State ?? WebSocketState.Closed, $"Qwen Realtime Wss Connection failed: {ex.Message}") ?? Task.CompletedTask);
            await CleanUpCurrentConnectionAsync("Qwen Realtime Wss Connection failed."); // Ensure resources are cleaned up on failure
            throw;
        }
    }
