...
public class AgentService : IAgentService
{
    ...
    public async Task<GetAgentsResponse> GetAgentsAsync(GetAgentsRequest request, CancellationToken cancellationToken)
    {
        ...
        var agentInfo = await GetAllAgentsAsync(agentTypes, request.AgentIds, request.PosServiceId, cancellationToken).ConfigureAwait(false);
        
        return new GetAgentsResponse { Data = agentInfo.OrderBy(x => x.CreatedDate).ToList() };
    }

    private async Task<List<AgentPreviewDto>> GetAllAgentsAsync(List<AgentType> agentTypes, List<int> agentIds, int? posServiceId, CancellationToken cancellationToken)
    {
        ...
        foreach (var type in types)
        {
            ...
            var task = (Task)genericMethod.Invoke(_agentDataProvider, [type.AgentType, agentIds, posServiceId, cancellationToken]);
