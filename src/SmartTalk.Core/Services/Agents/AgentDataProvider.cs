...
public interface IAgentDataProvider : IScopedDependency
{
    ...
    Task<List<AgentPreviewDto>> GetAgentsByAgentTypeAsync<T>(AgentType agentType, List<int> agentIds = null, int? posServiceId = null, CancellationToken cancellationToken = default) where T : class, IEntity<int>, IAgent;
}
...
public class AgentDataProvider : IAgentDataProvider
{
    ...
    public async Task<List<AgentPreviewDto>> GetAgentsByAgentTypeAsync<T>(
        AgentType agentType, List<int> agentIds = null, int? posServiceId = null, CancellationToken cancellationToken = default) where T : class, IEntity<int>, IAgent
    {
        var query = from agent in _repository.Query<Agent>()
            ...
            where agent.Type == agentType && (agentIds == null || agentIds.Count == 0 || agentIds.Contains(agent.Id)) 
                                          && (!posServiceId.HasValue || agent.PosServiceId == posServiceId.Value)
