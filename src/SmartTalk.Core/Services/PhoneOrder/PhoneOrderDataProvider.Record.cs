...
public partial interface IPhoneOrderDataProvider
{
    ...
    Task<PhoneOrderRecord> GetPhoneOrderRecordByOmePhoneAsync(string inComingCallNumber, DateTimeOffset callTime, CancellationToken cancellationToken);
}
...
public partial class PhoneOrderDataProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        var agentsQuery = from agent in _repository.QueryNoTracking<Agent>()
            join agentAssistant in _repository.QueryNoTracking<AgentAssistant>() on agent.Id equals agentAssistant.AgentId into agentAssistantGroups
            from agentAssistant in agentAssistantGroups.DefaultIfEmpty()
            join assistant in _repository.QueryNoTracking<Domain.AISpeechAssistant.AiSpeechAssistant>() on agentAssistant.AssistantId equals assistant.Id into assistantGroups
        ...
        var records = await query.OrderByDescending(record => record.CreatedDate).Take(1000).ToListAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<PhoneOrderRecord> GetPhoneOrderRecordByOmePhoneAsync(string inComingCallNumber, DateTimeOffset callTime, CancellationToken cancellationToken)
    {
