...
public partial class PhoneOrderService
{
    ...
    public async Task ReceivePhoneOrderRecordAsync(ReceivePhoneOrderRecordCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            new() { SessionId = Guid.NewGuid().ToString(), Restaurant = recordInfo.Restaurant, TranscriptionText = transcription, Url = fileUrl, CreatedDate = recordInfo.OrderDate.ToOffset(TimeSpan.FromHours(-14)) }
