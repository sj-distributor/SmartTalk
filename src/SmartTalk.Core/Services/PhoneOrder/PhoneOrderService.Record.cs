...
public partial class PhoneOrderService
{
    ...
    public async Task ReceivePhoneOrderRecordAsync(ReceivePhoneOrderRecordCommand command, CancellationToken cancellationToken)
    {
        ...
        if (await CheckOrderExistAsync(recordInfo.OrderDate.AddHours(-8), recordInfo.Restaurant, cancellationToken).ConfigureAwait(false)) return;
    ...
    private async Task<bool> CheckOrderExistAsync(DateTimeOffset createdDate, PhoneOrderRestaurant restaurant, CancellationToken cancellationToken)
    {
