                ...
                Messages = new List<CompletionsRequestMessageDto>
                {
                    new()
                    {
                        ...
                        Content = new CompletionsStringContent(
                            ...
                            "【重要】报告中出现的数字编号（如0500000150、INV-001等）即为订单号/Invoice单号，无论报告中使用「訂單號」「订单号」「单号」「Invoice」「INV-」哪种表述，都必须提取到 invoiceNumbers 字段中。\n" +
                            "每个 item 代表一条独立投诉，按问题分开输出。如果报告中描述了一家店的问题对应一个订单号，另一家店对应另一个订单号，必须拆成两条独立的 item，各自携带对应的订单号。\n" +
                            "所有字段有就填、没有就留空，不要臆造。\n" +
