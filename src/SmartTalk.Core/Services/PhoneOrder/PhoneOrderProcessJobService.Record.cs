...
public partial class PhoneOrderProcessJobService
{
    ...
    public async Task HandleReleasedSpeechMaticsCallBackAsync(string jobId, CancellationToken cancellationToken)
    {
        ...
        var record = await _phoneOrderDataProvider.GetPhoneOrderRecordByTranscriptionJobIdAsync(speechMaticsJob.JobId, cancellationToken).ConfigureAwait(false);
        ...
        string callTo, string callSubjectCn, string callSubjectEn, string currentTime, byte[] audioContent, CancellationToken cancellationToken) 
    {
        ...
        if (agent.Type == AgentType.Sales)
        {
            ...
            if (sales != null)
            {
                ...
                askItemsJson = JsonConvert.SerializeObject(simplifiedItems, Formatting.Indented);
    ...
    private async Task RefineOrderByAiAsync(ExtractedOrderDto storeOrder, string soldToId, CancellationToken cancellationToken)
    {
        ...
        var systemPrompt =
            ...
            "4. 系统草稿单中已提供字段 IsProcessed，true 表示该物料在 hifood 那边已经打单。\n\n" +
            ...
            "    - Unit = 优先取草稿单 Unit。\n" +
            "    - 即使草稿单 IsProcessed = true，只要该物料在本次通话中被匹配到并发生变动，仍然必须输出。\n\n" +
            "- **若在草稿单中未找到匹配项（新增）**：\n" +
