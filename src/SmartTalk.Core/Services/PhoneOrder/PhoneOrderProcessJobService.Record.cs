...
public partial class PhoneOrderProcessJobService
{
    ...
    private async Task RefineOrderByAiAsync(ExtractedOrderDto storeOrder, string soldToId, Domain.AISpeechAssistant.AiSpeechAssistant aiSpeechAssistant,  List<(string Material, string MaterialDesc, DateTime? invoiceDate)> historyItems, int recordId, CancellationToken cancellationToken)
    {
        ...
        var systemPrompt =
            "你是「电话下单意图裁决助手」，负责判断客户最终要执行的订单结果。\n\n" +
            "【优先级】\n" +
            "1. 本次通话内容优先\n" +
            "2. 草稿单仅用于判断是否存在、是否被取消，不可覆盖本次通话指令\n" +
            "3. 历史分析报告仅提供上下文参考，不可覆盖本次通话指令\n\n" +
            "【任务】\n" +
            "1. 判断是否为整单取消（IsDeleteWholeOrder）\n" +
            "2. 判断是否为撤销取消（IsUndoCancel）\n" +
            "3. 合并同一物料的多次加减单，计算最终数量\n" +
            "4. 确保名称拼接格式正确，并遵守加减单规则（示例：鸡胸肉#1箱+2）\n\n" +
            "【规则】\n" +
            "1. 整单取消：只有当客户明确表示取消所有物品时，才能设置 IsDeleteWholeOrder = true，Orders 必须为空数组。\n" +
            "2. 撤销取消：当客户要求恢复之前取消的物料时，设置 IsUndoCancel = true。\n" +
            "3. 物料合并：同一物料的多个修改（加减单）需合并为一条，最终输出合并后的数量。\n" +
            "4. 单个物料取消：如果客户明确取消某物料，设置 MarkForDelete = true，并确保取消的物料出现在最终结果中。\n" +
            "5. 物料名称：必须体现加减过程，格式：物料名#数量单位+变化（如：鸡胸肉#1箱+2）。\n" +
            "6. 如果某物料未在通话中修改或取消，但在草稿单中存在，保留并输出原数量。\n\n" +
            "【输出格式】\n" +
            "{\n" +
            "  \"StoreName\": \"\",\n" +
            "  \"StoreNumber\": \"\",\n" +
