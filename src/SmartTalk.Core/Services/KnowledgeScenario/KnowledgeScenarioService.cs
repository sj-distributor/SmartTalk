...
public class KnowledgeScenarioService : IKnowledgeScenarioService
{
    ...
    public async Task<UpdateKnowledgeSceneFolderResponse> UpdateKnowledgeSceneFolderAsync(UpdateKnowledgeSceneFolderCommand command, CancellationToken cancellationToken)
    {
        ...
        var folder = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneFoldersAsync(id: command.Id, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
    ...
    public async Task<DeleteKnowledgeSceneFolderResponse> DeleteKnowledgeSceneFolderAsync(DeleteKnowledgeSceneFolderCommand command, CancellationToken cancellationToken)
    {
        ...
        var scenes = await _knowledgeScenarioDataProvider.GetKnowledgeScenesAsync(folderId: command.Id, cancellationToken: cancellationToken).ConfigureAwait(false);
        var sceneIds = scenes.Select(x => x.Id).ToList();
        var knowledgeSceneItems = await _knowledgeScenarioDataProvider.GetKnowledgeSceneItemsBySceneIdsAsync(sceneIds, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<AddKnowledgeSceneResponse> AddKnowledgeSceneAsync(AddKnowledgeSceneCommand command, CancellationToken cancellationToken)
    {
        ...
        var folder = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneFoldersAsync(id: command.FolderId, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
    ...
    public async Task<UpdateKnowledgeSceneResponse> UpdateKnowledgeSceneAsync(UpdateKnowledgeSceneCommand command, CancellationToken cancellationToken)
    {
        ...
        var scene = (await _knowledgeScenarioDataProvider.GetKnowledgeScenesByIdsAsync([command.Id], cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
        ...
        var snapshotSceneItems = await _knowledgeScenarioDataProvider.GetKnowledgeSceneItemsBySceneIdAsync(scene.Id, cancellationToken: cancellationToken).ConfigureAwait(false);

        _backgroundJobClient.Enqueue<IKnowledgeScenarioProcessJobService>(
            x => x.HandleSceneUpdatedAsync(new KnowledgeSceneUpdatedSnapshotDto
            {
                SceneId = scene.Id,
                ...
                SceneItems = snapshotSceneItems.Select(item => new KnowledgeSceneUpdatedSnapshotItem
                {
                    SceneItemId = item.Id,
                    Name = item.Name,
                    Type = item.Type,
    ...
    public async Task<DeleteKnowledgeSceneResponse> DeleteKnowledgeSceneAsync(DeleteKnowledgeSceneCommand command, CancellationToken cancellationToken)
    {
        ...
        var languageMappings = await _knowledgeScenarioDataProvider.GetKnowledgeSceneLanguageMappingsAsync(sceneIds: [scene.Id], isActive: true, cancellationToken: cancellationToken).ConfigureAwait(false);
        var sceneItems = await _knowledgeScenarioDataProvider.GetKnowledgeSceneItemsBySceneIdAsync(scene.Id, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        var sceneCompanies = await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([scene.Id], cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    private void EnqueueKnowledgeCleanupJob(List<KnowledgeSceneLanguageMapping> removedMappings)
    {
        ...
        {
            _backgroundJobClient.Enqueue<IAiSpeechAssistantProcessJobService>(x => x.CleanupAiSpeechAssistantKnowledgeByLanguageAsync(command, CancellationToken.None));
        }
    }
    ...
    private async Task<bool> SyncKnowledgeSceneItemsAsync(KnowledgeScene scene, List<KnowledgeSceneItemDto> items, CancellationToken cancellationToken)
    {
        ...
        var existingItems = await _knowledgeScenarioDataProvider.GetKnowledgeSceneItemsBySceneIdAsync(scene.Id, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        var itemsToUpdate = new List<KnowledgeSceneItem>();
        foreach (var item in targetItems)
        {
            if (item.Id > 0)
            {
                ...
                var updatedContent = item.Content;
        ...
        if (itemsToUpdate.Count != 0)
            await _knowledgeScenarioDataProvider.UpdateKnowledgeSceneItemsAsync(itemsToUpdate, false, cancellationToken).ConfigureAwait(false);
    ...
    {
        var folders = await _knowledgeScenarioDataProvider.GetKnowledgeSceneFoldersAsync(keyword: request.Keyword, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<GetKnowledgeScenesResponse> GetKnowledgeScenesAsync(GetKnowledgeScenesRequest request, CancellationToken cancellationToken)
    {
        ...
        var scenes = await _knowledgeScenarioDataProvider.GetKnowledgeScenesAsync(folderId: request.FolderId, keyword: request.Keyword, cancellationToken: cancellationToken).ConfigureAwait(false);
        var sceneItems = await _knowledgeScenarioDataProvider.GetKnowledgeSceneItemsBySceneIdsAsync(scenes.Select(x => x.Id).ToList(), cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<GetKnowledgeSceneResponse> GetKnowledgeSceneAsync(GetKnowledgeSceneRequest request, CancellationToken cancellationToken)
    {
        ...
        var scene = (await _knowledgeScenarioDataProvider.GetKnowledgeScenesByIdsAsync([request.Id], cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();

        if (scene == null)
            throw new Exception($"GetKnowledgeScene Scene [{request.Id}] does not exist.");

        var knowledges = await _knowledgeScenarioDataProvider.GetKnowledgeSceneItemsBySceneIdAsync(request.Id, cancellationToken: cancellationToken).ConfigureAwait(false);
        var sceneDto = _mapper.Map<KnowledgeSceneDto>(scene);
    ...
    public async Task<GetKnowledgeSceneHistoryResponse> GetKnowledgeSceneHistoryAsync(GetKnowledgeSceneHistoryRequest request, CancellationToken cancellationToken)
    {
        ...
        var scene = (await _knowledgeScenarioDataProvider.GetKnowledgeScenesByIdsAsync([request.SceneId], cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
        if (scene == null)
            throw new Exception($"GetKnowledgeSceneHistory Scene [{request.SceneId}] does not exist.");

        var (count, histories) = await _knowledgeScenarioDataProvider.GetKnowledgeSceneHistoriesAsync(sceneId: request.SceneId, pageIndex: request.PageIndex, pageSize: request.PageSize, cancellationToken: cancellationToken).ConfigureAwait(false);
        var historyItems = await _knowledgeScenarioDataProvider.GetKnowledgeSceneHistoryItemsAsync(histories.Select(x => x.Id).ToList(), cancellationToken).ConfigureAwait(false);
    ...
    private async Task<(List<KnowledgeSceneCompany> SceneCompanies, List<KnowledgeSceneCompany> SceneStoreApplications)> LoadMarketSceneRowsAsync(int companyId, int storeId, CancellationToken cancellationToken)
    {
        var rows = await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesAsync(companyId: companyId, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    private async Task<List<KnowledgeScene>> LoadAuthorizedScenesAsync(List<KnowledgeSceneCompany> sceneCompanies, CancellationToken cancellationToken)
    {
        ...
        return await _knowledgeScenarioDataProvider.GetKnowledgeScenesByIdsAsync(sceneIds, cancellationToken: cancellationToken).ConfigureAwait(false);
    }
    ...
    private static List<KnowledgeScene> FilterMarketScenes(List<KnowledgeScene> scenes, List<KnowledgeSceneCompany> storeApplications, string keyword, KnowledgeSceneMarketType marketType)
    {
        ...
        {
            return list.Where(x => IsApplied(storeApplications, x.Id)).ToList();
        }
        
        return list.Where(x => x.Status == KnowledgeSceneStatus.Published || IsApplied(storeApplications, x.Id)).ToList();
    }
    ...
    public async Task<GetKnowledgeSceneCompaniesResponse> GetKnowledgeSceneCompaniesAsync(GetKnowledgeSceneCompaniesRequest request, CancellationToken cancellationToken)
    {
        ...
        var sceneCompanies = await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([request.SceneId], isCompanyAuthorization: true, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<SaveKnowledgeSceneRelatedKnowledgesResponse> SaveKnowledgeSceneRelatedKnowledgesAsync(SaveKnowledgeSceneRelatedKnowledgesCommand command, CancellationToken cancellationToken)
    {
        ...
        var scene = (await _knowledgeScenarioDataProvider.GetKnowledgeScenesByIdsAsync([command.SceneId], cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
    ...
    private async Task EnsureSceneAppliedToStoreAsync(int sceneId, int storeId, CancellationToken cancellationToken)
    {
        ...
        var companyAuthorization = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync(
            [sceneId], companyId: store.CompanyId, isCompanyAuthorization: true, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();

        if (companyAuthorization == null)
            throw new Exception($"Scene [{sceneId}] is not authorized for Company [{store.CompanyId}].");

        var storeApplication = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync(
            [sceneId], storeId: storeId, isCompanyAuthorization: false, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
    ...
    public async Task<SaveKnowledgeSceneCompaniesResponse> SaveKnowledgeSceneCompaniesAsync(SaveKnowledgeSceneCompaniesCommand command, CancellationToken cancellationToken)
    {
        ...
        var currentCompanies = await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([command.SceneId], isCompanyAuthorization: true, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        var storeApplicationsToDelete = companiesToDelete.Count == 0
            ? []
            : await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([command.SceneId], isCompanyAuthorization: false, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        var latestCompanies = await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([command.SceneId], isCompanyAuthorization: true, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<UpdateKnowledgeSceneCompanyResponse> UpdateKnowledgeSceneCompanyAsync(UpdateKnowledgeSceneCompanyCommand command, CancellationToken cancellationToken)
    {
        ...
        var sceneCompany = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([command.SceneId], companyId: command.CompanyId, isCompanyAuthorization: true, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
        ...
        var sceneStore = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneCompaniesBySceneIdsAsync([command.SceneId], storeId: command.StoreId, isCompanyAuthorization: false, cancellationToken: cancellationToken).ConfigureAwait(false)).FirstOrDefault();
        if (sceneStore == null)
        {
        ...
        if (sceneStore.Id <= 0)
            await _knowledgeScenarioDataProvider.AddKnowledgeSceneCompaniesAsync(new List<KnowledgeSceneCompany> { sceneStore }, true, cancellationToken).ConfigureAwait(false);
        else
    ...
    public async Task<UpdateKnowledgeSceneHistoryResponse> UpdateKnowledgeSceneHistoryAsync(UpdateKnowledgeSceneHistoryCommand command, CancellationToken cancellationToken)
    {
        ...
        var history = (await _knowledgeScenarioDataProvider.GetKnowledgeSceneHistoriesAsync(historyId: command.HistoryId, cancellationToken: cancellationToken).ConfigureAwait(false)).Histories.FirstOrDefault();
        ...
        history.Brief = string.IsNullOrWhiteSpace(command.Brief) ? "未命名改動" : command.Brief;
    ...
    {
        var (_, histories) = await _knowledgeScenarioDataProvider.GetKnowledgeSceneHistoriesAsync(sceneId: sceneId, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        SaveKnowledgeSceneLanguageMappingsCommand command, CancellationToken cancellationToken)
    {
        ...
        mappingsToDeactivate.ForEach(x => x.IsActive = false);
