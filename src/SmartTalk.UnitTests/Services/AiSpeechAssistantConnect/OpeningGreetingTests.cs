...
public class OpeningGreetingTests
{
    ...
    [Fact]
    public async Task SendOpeningGreetingOnceAsync_CallInstruction_ReplacesGreetingAndSendsOnce()
    {
        var context = CreateContext("Default greeting");
        context.Instruction = "Introduce yourself as the customer's representative.";
        var sentMessages = new List<string>();
        var actions = new RealtimeAiSessionActions
        {
            SendTextToProviderAsync = message =>
            {
        ...
        Assert.Equal([context.Instruction], sentMessages);
    }
