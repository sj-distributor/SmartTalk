...
public class PhoneOrderController : ControllerBase
{
    ...
    public async Task HandleMediaStream()
    {
        if (HttpContext.WebSockets.IsWebSocketRequest)
        {
            ...
            await HandleWebSocketAsync(clientWebSocket);
        }
        else
        {
    ...
    private async Task HandleWebSocketAsync(WebSocket twilioWebSocket)
    {
        Log.Information("Client connected");

        using var openAiWebSocket = new ClientWebSocket();
        openAiWebSocket.Options.SetRequestHeader("Authorization", $"Bearer {_openAiApiKey}");
